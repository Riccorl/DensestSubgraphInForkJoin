...
public class UndirectedJGraph extends RecursiveTask<Graph<Integer, DefaultEdge>>{
    ...
    public UndirectedJGraph(SortedMap<Integer, Integer> rows) {
        this.rows = rows;
        this.high = this.rows.keySet().size();
        this.cutoff = this.high/16;
    }

    public UndirectedJGraph(SortedMap<Integer, Integer> rows, int high, int cutoff) {
        this.rows = rows;
        this.high = high;
        this.cutoff = cutoff;
    }
    ...
    protected Graph<Integer, DefaultEdge> compute() {
        ...
        UndirectedJGraph left = new UndirectedJGraph(lowList, lowList.keySet().size(), this.cutoff);
        UndirectedJGraph right = new UndirectedJGraph(lowList, highList.keySet().size(), this.cutoff);
